...
app.post('/generate-edit-plan', async (req, res) => {
    try {
        const { mask, change, lockedStyle, currentDescription } = req.body;
        if (!mask || !change) {
            return res.status(400).json({ error: 'Faltan parámetros: mask, change' });
        }
        // Construir el prompt para el LLM
        const instruction = `
Eres un asistente experto en edición de cómics secuenciales. Tu tarea es generar un plan de edición detallado y estructurado basado en la solicitud del usuario.
Instrucciones:
- El usuario quiere modificar el elemento "${mask}" para que ahora "${change}".
- El estilo artístico fijo es: "${lockedStyle}".
- La descripción actual de la escena es: "${currentDescription || 'No disponible'}".
- Genera un plan claro con secciones: Objetivo, Detalles, Advertencias.
- Sé específico sobre anatomía, expresiones, iluminación y composición.
- Enfócate en lo que DEBE cambiar y lo que NO debe cambiar bajo ninguna circunstancia.
...
...
`;
        console.log(`[Backend Log] /generate-edit-plan: Generando plan para: "${mask}" -> "${change}"`);
        // Configurar el modelo de texto (Gemini 2.5 Flash Lite es ideal para esto)
        const model = genAI.getGenerativeModel({
            model: "gemini-2.5-flash", // Usa el modelo de texto, no el de imagen
            generationConfig: {
                maxOutputTokens: 1024,
                temperature: 0.3, // Para respuestas más precisas y menos creativas
            }
        });
        const result = await model.generateContent([instruction]);
        const response = await result.response;
        const planText = response.text().trim();
        console.log(`[Backend Log] /generate-edit-plan: Plan generado exitosamente. Longitud: ${planText.length}`);
        res.json({ plan: planText });
    } catch (error) {
        console.error('[Backend Error] Error en /generate-edit-plan:', error);
        let errorMessage = error.message;
        if (error.message.includes('API key not valid')) {
            errorMessage = 'Clave de API de Gemini no válida.';
        }
        res.status(500).json({ error: `Error generando plan de edición: ${errorMessage}` });
    }
});
